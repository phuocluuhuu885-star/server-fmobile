...
const { sendEmail, generateConfirmationCode, generateExpirationTime } = require("../utils/NodemailerService");
	...
	try {
		...
		if (existingUser) {
			...
			if (existingUser.isVerify == false) {
				...
				await sendEmail(email, "Confirmation Code", `Your confirmation code: ${newConfirmationCode}`);
				return res.status(200).json({
					code: 200,
					message: "Email already exists, resend confirmation code successfully",
				});
			}

			return res.status(409).json({ code: 409, message: "Email already exists", isExit: true });
		}
		...
		await sendEmail(email, "Confirmation Code", `Your confirmation code: ${confirmationCode}`);
		return res.status(200).json({
			code: 200,
...
const login = async (req, res, next) => {
	try {
		...
		if (user.isVerify == false) {
			return res.status(401).json({ code: 401, message: "Unauthenticated email", isVerify: false });
		}
...
const verifyEmail = async (req, res, next) => {
	try {
		...
		if (!user) {
			return res.status(400).json({ code: 400, message: "Mã xác nhận không hợp lệ" });
		}

		const now = new Date();
		if (now > user.confirmationExpiration) {
			return res.status(400).json({ code: 400, message: "Mã xác nhận đã hết hạn" });
		}
		...
		return res.status(200).json({ code: 200, message: "Xác nhận email thành công" });
	} catch (error) {
		return res.status(500).json({ code: 500, message: error.message });
	}
};
...
const resendConfirmationCode = async (req, res, next) => {
	try {
		...
		sendEmail(email, "Confirmation Code", `Your confirmation code: ${confirmationCode}`);
		return res.status(200).json({
			code: 200,
